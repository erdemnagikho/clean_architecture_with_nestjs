...
export class CreateProductHandler implements ICommandHandler<CreateProductCommand> {
    ...
    async execute(command: CreateProductCommand): Promise<void> {
        const existingBySku = await this.productRepository.findBySku(Sku.create(command.sku));
        if (existingBySku) {
        ...
        const existingByName = await this.productRepository.findByName(command.name);
        if (existingByName) {
            throw new ApplicationException(
